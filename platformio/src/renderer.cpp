...
// daily forecast precipitation
  ...
  { // 1 alert
    ...
    display.drawInvertedBitmap(196, 8, getAlertBitmap48(cur_alert), 48, 48,
                               ACCENT_COLOR);
    // getAlertBitmap matches on lowercase event; display may use tags if CJK
    const String eventText = getAlertDisplayText(cur_alert);

    display.setFont(&FONT_14pt8b);
    if (getStringWidth(eventText) <= max_w)
    { // Fits on a single line, draw along bottom
      drawString(196 + 48 + 4, 24 + 8 - 12 + 20 + 1, eventText, LEFT);
    }
    else
    { // use smaller font
      display.setFont(&FONT_12pt8b);
      if (getStringWidth(eventText) <= max_w)
      { // Fits on a single line with smaller font, draw along bottom
        drawString(196 + 48 + 4, 24 + 8 - 12 + 17 + 1, eventText, LEFT);
      }
      else
      { // Does not fit on a single line, draw higher to allow room for 2nd line
        drawMultiLnString(196 + 48 + 4, 24 + 8 - 12 + 17 - 11,
                          eventText, LEFT, max_w, 2, 23);
      }
    }
  } // end 1 alert
  else
                   ...
                   int rssi, uint32_t batVoltage)
{
  ...
  {
    const String statusSafe = sanitizeAsciiForDisplay(statusStr);
    drawString(pos, DISP_HEIGHT - 1 - 2, statusSafe, RIGHT, dataColor);
    pos -= getStringWidth(statusSafe) + 24;
